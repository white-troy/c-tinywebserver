...
//可预留的空间：读过的=被用过的，所以返回读下标就等于剩下的空间
...
//取出所有数据，buffer归零 ，读写下标归零，在别的函数中使用到
...
ssize_t Buffer::ReadFd(int fd, int* Errno){
    char buff[65535];//栈区
    struct iovec iov[2];
    size_t writeable = WritableBytes();//记录下能写多少
    // 分散读， 保证数据全部读完
    ...
    if(len<0){
        *Errno = errno;
        ...
        Append(buff,static_cast<size_t>(len-writeable));//剩余长度
    }
    return len;
}
