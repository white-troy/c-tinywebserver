 ...
 * @Date: 2023-12-18 19:55:49
 * @LastEditors: K2SO4 zs1115996793@qq.com
 * @LastEditTime: 2023-12-18 20:24:11
 * @FilePath: \c-tinywebserver\server\webserver.cpp
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AEMS
 */
#include"webserver.h"
...
WebServer::WebServer(
    ...
    port_(port), timeoutMS_(timeoutMS), isClose_(isClose),
    ...
    //初始化操作
    SqlConnPool::Instance()->Init("localhost",sqlPort,sqlUser,sqlPwd,dbName,connPoolNum);//连接池单例初始化
    //初始化事件和初始化socket(监听)
    InitEvenMode_(trigMode);
...
void WebServer::InitEvenMode_(int trigMode){
    listenEvent_ = EPOOLRDHUP;//检测socket关闭
...
void WebServer::Start(){
    ...
    while(!isClose_){
        if(timeoutMS_ > 0){
            //获取下一次的超时等待时间
            //至少这个事件才会有用户过期，每次关闭超时连接则需要有新的请求
        ...
        for(int i = 0;i<eventCnt;i++){
            ...
            else if(events & (EPOLLRDHUP|EPOLLHUB|EPOLLERR)){
...
void WebServer::DealRead_(HttpConn* client){
    assert(client);
    ExtentTime_(client);
    threadpool_->AddTask(std::bind(&WebServer::OnRead_,this,client));//bind将参数和函数绑定
}
...
void WebServer::OnProcess_(HttpConn* client){
    //调用process进行逻辑处理
    if(client->process()){
        //根据返回的信息将fd置为EPOLLOUT（写）或EPOLLIN（读）
        //读完事件就跟内核说可以写
